...
$Page->showMessage();
?>
<form name="fsectorsadd" id="fsectorsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<span id="el_sectors_sector_category_id">
    <select
        ...
        data-select2-id="fsectorsadd_x_sector_category_id"
        <?php } ?>
        data-table="sectors"
...
<?php if (!$Page->sector_category_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fsectorsadd", function() {
    var options = { name: "x_sector_category_id", selectId: "fsectorsadd_x_sector_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fsectorsadd.lists.sector_category_id?.lookupOptions.length) {
        options.data = { id: "x_sector_category_id", form: "fsectorsadd" };
    } else {
        options.ajax = { id: "x_sector_category_id", form: "fsectorsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fsectorsadd", "editor"], function() {
    ew.createEditor("fsectorsadd", "x_description", 35, 4, <?= $Page->description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsectorsadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsectorsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
