...
$Page->showMessage();
?>
<form name="fprojectsadd" id="fprojectsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<span id="el_projects_project_category_id">
    <select
        ...
        data-select2-id="fprojectsadd_x_project_category_id"
        <?php } ?>
        data-table="projects"
...
loadjs.ready("fprojectsadd", function() {
    var options = { name: "x_project_category_id", selectId: "fprojectsadd_x_project_category_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprojectsadd.lists.project_category_id?.lookupOptions.length) {
        options.data = { id: "x_project_category_id", form: "fprojectsadd" };
    } else {
        options.ajax = { id: "x_project_category_id", form: "fprojectsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_projects_project_provider_id">
    <select
        ...
        data-select2-id="fprojectsadd_x_project_provider_id"
        <?php } ?>
        data-table="projects"
...
<script>
loadjs.ready("fprojectsadd", function() {
    var options = { name: "x_project_provider_id", selectId: "fprojectsadd_x_project_provider_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprojectsadd.lists.project_provider_id?.lookupOptions.length) {
        options.data = { id: "x_project_provider_id", form: "fprojectsadd" };
    } else {
        options.ajax = { id: "x_project_provider_id", form: "fprojectsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_projects_project_status_id">
    <select
        ...
        data-select2-id="fprojectsadd_x_project_status_id"
        <?php } ?>
        data-table="projects"
...
<script>
loadjs.ready("fprojectsadd", function() {
    var options = { name: "x_project_status_id", selectId: "fprojectsadd_x_project_status_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fprojectsadd.lists.project_status_id?.lookupOptions.length) {
        options.data = { id: "x_project_status_id", form: "fprojectsadd" };
    } else {
        options.ajax = { id: "x_project_status_id", form: "fprojectsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready("fprojectsadd", function() {
    fprojectsadd.createAutoSuggest(Object.assign({"id":"x_funding_source_id","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->funding_source_id->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.projects.fields.funding_source_id.autoSuggestOptions));
});
</script>
...
loadjs.ready(["fprojectsadd", "editor"], function() {
    ew.createEditor("fprojectsadd", "x_project_description", 35, 4, <?= $Page->project_description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
loadjs.ready(["fprojectsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fprojectsadd", "x_project_start", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fprojectsadd", "editor"], function() {
    ew.createEditor("fprojectsadd", "x_project_html", 0, 0, <?= $Page->project_html->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<input type="<?= $Page->slug->getInputTextType() ?>" name="x_slug" id="x_slug" data-table="projects" data-field="x_slug" value="<?= $Page->slug->EditValue ?>" data-page="1" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->slug->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->slug->formatPattern()) ?>"<?= $Page->slug->editAttributes() ?> aria-describedby="x_slug_help">
...
loadjs.ready(["fprojectsadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fprojectsadd", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("project_investors", explode(",", $Page->getCurrentDetailTable())) && $project_investors->DetailAdd) {
?>
...
?>
<?php
    if (in_array("project_files", explode(",", $Page->getCurrentDetailTable())) && $project_files->DetailAdd) {
?>
