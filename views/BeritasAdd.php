...
$Page->showMessage();
?>
<form name="fberitasadd" id="fberitasadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<span id="el_beritas_kategori_berita_id">
    <select
        ...
        data-select2-id="fberitasadd_x_kategori_berita_id"
        <?php } ?>
        data-table="beritas"
...
<?php if (!$Page->kategori_berita_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fberitasadd", function() {
    var options = { name: "x_kategori_berita_id", selectId: "fberitasadd_x_kategori_berita_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fberitasadd.lists.kategori_berita_id?.lookupOptions.length) {
        options.data = { id: "x_kategori_berita_id", form: "fberitasadd" };
    } else {
        options.ajax = { id: "x_kategori_berita_id", form: "fberitasadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fberitasadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fberitasadd", "x_tanggal_terbit", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fberitasadd", "editor"], function() {
    ew.createEditor("fberitasadd", "x_isi_berita", 35, 4, <?= $Page->isi_berita->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<input type="hidden" name="fa_x_gbr_berita" id= "fa_x_gbr_berita" value="0">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fberitasadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fberitasadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
