...
?>
<main class="edit">
<form name="frole_has_permissionsedit" id="frole_has_permissionsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { role_has_permissions: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var frole_has_permissionsedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("frole_has_permissionsedit")
        .setPageId("edit")
...
<input type="hidden" data-table="role_has_permissions" data-field="x_permission_id" data-hidden="1" data-old name="o_permission_id" id="o_permission_id" value="<?= HtmlEncode($Page->permission_id->OldValue ?? $Page->permission_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="role_has_permissions" data-field="x_role_id" data-hidden="1" data-old name="o_role_id" id="o_role_id" value="<?= HtmlEncode($Page->role_id->OldValue ?? $Page->role_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="frole_has_permissionsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="frole_has_permissionsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
