...
?>
<main class="edit">
<form name="fmodel_has_permissionsedit" id="fmodel_has_permissionsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { model_has_permissions: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fmodel_has_permissionsedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fmodel_has_permissionsedit")
        .setPageId("edit")
...
<?php if ($Page->model_id->Visible) { // model_id ?>
    <div id="r_model_id"<?= $Page->model_id->rowAttributes() ?>>
        <label id="elh_model_has_permissions_model_id" for="x_model_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->model_id->caption() ?><?= $Page->model_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->model_id->cellAttributes() ?>>
<span id="el_model_has_permissions_model_id">
<input type="<?= $Page->model_id->getInputTextType() ?>" name="x_model_id" id="x_model_id" data-table="model_has_permissions" data-field="x_model_id" value="<?= $Page->model_id->EditValue ?>" size="30" placeholder="<?= HtmlEncode($Page->model_id->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->model_id->formatPattern()) ?>"<?= $Page->model_id->editAttributes() ?> aria-describedby="x_model_id_help">
<?= $Page->model_id->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->model_id->getErrorMessage() ?></div>
<input type="hidden" data-table="model_has_permissions" data-field="x_model_id" data-hidden="1" data-old name="o_model_id" id="o_model_id" value="<?= HtmlEncode($Page->model_id->OldValue ?? $Page->model_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fmodel_has_permissionsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fmodel_has_permissionsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
