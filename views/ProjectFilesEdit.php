...
?>
<main class="edit">
<form name="fproject_filesedit" id="fproject_filesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { project_files: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fproject_filesedit;
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id"<?= $Page->id->rowAttributes() ?>>
        <label id="elh_project_files_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->id->cellAttributes() ?>>
<span id="el_project_files_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
<input type="hidden" data-table="project_files" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_project_files_project_id">
    <select
        ...
        data-select2-id="fproject_filesedit_x_project_id"
        <?php } ?>
...
<?php if (!$Page->project_id->IsNativeSelect) { ?>
<script>
loadjs.ready("fproject_filesedit", function() {
    var options = { name: "x_project_id", selectId: "fproject_filesedit_x_project_id" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fproject_filesedit.lists.project_id?.lookupOptions.length) {
        options.data = { id: "x_project_id", form: "fproject_filesedit" };
    } else {
        options.ajax = { id: "x_project_id", form: "fproject_filesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<input type="hidden" name="fa_x_file_path" id= "fa_x_file_path" value="<?= (Post("fa_x_file_path") == "0") ? "0" : "1" ?>">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fproject_filesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fproject_filesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
