...
$Page->showMessage();
?>
<form name="fpermissions2add" id="fpermissions2add" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="on">
...
<?php if ($Page->guard_name->Visible) { // guard_name ?>
    <div id="r_guard_name"<?= $Page->guard_name->rowAttributes() ?>>
...
<input type="<?= $Page->guard_name->getInputTextType() ?>" name="x_guard_name" id="x_guard_name" data-table="permissions2" data-field="x_guard_name" value="<?= $Page->guard_name->EditValue ?>" size="30" maxlength="255" placeholder="<?= HtmlEncode($Page->guard_name->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->guard_name->formatPattern()) ?>"<?= $Page->guard_name->editAttributes() ?> aria-describedby="x_guard_name_help">
...
<input type="<?= $Page->created_at->getInputTextType() ?>" name="x_created_at" id="x_created_at" data-table="permissions2" data-field="x_created_at" value="<?= $Page->created_at->EditValue ?>" placeholder="<?= HtmlEncode($Page->created_at->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->created_at->formatPattern()) ?>"<?= $Page->created_at->editAttributes() ?> aria-describedby="x_created_at_help">
...
loadjs.ready(["fpermissions2add", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpermissions2add", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fpermissions2add", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpermissions2add", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpermissions2add"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpermissions2add" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
