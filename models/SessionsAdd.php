 ...
 * Page class
 */
class SessionsAdd extends Sessions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SessionsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "SessionsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("SessionsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "SessionsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "SessionsView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "SessionsList") {
                            ...
                            $returnUrl = "SessionsList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'last_activity' first before field var 'x_last_activity'
        $val = $CurrentForm->hasValue("last_activity") ? $CurrentForm->getValue("last_activity") : $CurrentForm->getValue("x_last_activity");
        if (!$this->last_activity->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->last_activity->Visible = false; // Disable update for API request
            } else {
                $this->last_activity->setFormValue($val, true, $validate);
            }
        }
    }
    ...
    public function renderRow()
    {
        ...
        // payload
        $this->payload->RowCssClass = "row";

        // last_activity
        $this->last_activity->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // last_activity
            $this->last_activity->setupEditAttributes();
            $this->last_activity->EditValue = HtmlEncode($this->last_activity->CurrentValue);
            $this->last_activity->PlaceHolder = RemoveHtml($this->last_activity->caption());
            if (strval($this->last_activity->EditValue) != "" && is_numeric($this->last_activity->EditValue)) {
                $this->last_activity->EditValue = FormatNumber($this->last_activity->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->payload->Visible && $this->payload->Required) {
        ...
        if ($this->last_activity->Visible && $this->last_activity->Required) {
            if (!$this->last_activity->IsDetailKey && EmptyValue($this->last_activity->FormValue)) {
                $this->last_activity->addErrorMessage(str_replace("%s", $this->last_activity->caption(), $this->last_activity->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->last_activity->FormValue)) {
            $this->last_activity->addErrorMessage($this->last_activity->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // last_activity
        $this->last_activity->setDbValueDef($rsnew, $this->last_activity->CurrentValue, false);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SessionsList"), "", $this->TableVar, true);
