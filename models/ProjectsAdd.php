...
class ProjectsAdd extends Projects
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("project_investors", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("ProjectFilesGrid");
        if (in_array("project_files", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // project_category_id
        $this->project_category_id->setDbValueDef($rsnew, $this->project_category_id->CurrentValue, false);

        // project_provider_id
        $this->project_provider_id->setDbValueDef($rsnew, $this->project_provider_id->CurrentValue, false);

        // project_status_id
        $this->project_status_id->setDbValueDef($rsnew, $this->project_status_id->CurrentValue, false);

        // funding_source_id
        $this->funding_source_id->setDbValueDef($rsnew, $this->funding_source_id->CurrentValue, false);

        // project_name
        $this->project_name->setDbValueDef($rsnew, $this->project_name->CurrentValue, false);

        // project_description
        $this->project_description->setDbValueDef($rsnew, $this->project_description->CurrentValue, false);

        // project_budget
        $this->project_budget->setDbValueDef($rsnew, $this->project_budget->CurrentValue, false);

        // project_target
        $this->project_target->setDbValueDef($rsnew, $this->project_target->CurrentValue, false);

        // project_start
        $this->project_start->setDbValueDef($rsnew, UnFormatDateTime($this->project_start->CurrentValue, $this->project_start->formatPattern()), false);

        // project_duration
        $this->project_duration->setDbValueDef($rsnew, $this->project_duration->CurrentValue, false);

        // project_html
        $this->project_html->setDbValueDef($rsnew, $this->project_html->CurrentValue, false);

        // project_headgbr
        $this->project_headgbr->setDbValueDef($rsnew, $this->project_headgbr->CurrentValue, false);
