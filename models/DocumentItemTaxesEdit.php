 ...
 * Page class
 */
class DocumentItemTaxesEdit extends DocumentItemTaxes
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentItemTaxesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentItemTaxesEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("DocumentItemTaxesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "DocumentItemTaxesList") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "DocumentItemTaxesList") {
                            ...
                            $returnUrl = "DocumentItemTaxesList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'departement__d' first before field var 'x_departement__d'
        $val = $CurrentForm->hasValue("departement__d") ? $CurrentForm->getValue("departement__d") : $CurrentForm->getValue("x_departement__d");
        if (!$this->departement__d->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->departement__d->Visible = false; // Disable update for API request
            } else {
                $this->departement__d->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'document_item_id' first before field var 'x_document_item_id'
        $val = $CurrentForm->hasValue("document_item_id") ? $CurrentForm->getValue("document_item_id") : $CurrentForm->getValue("x_document_item_id");
        if (!$this->document_item_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->document_item_id->Visible = false; // Disable update for API request
            } else {
                $this->document_item_id->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'tax_id' first before field var 'x_tax_id'
        $val = $CurrentForm->hasValue("tax_id") ? $CurrentForm->getValue("tax_id") : $CurrentForm->getValue("x_tax_id");
        if (!$this->tax_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->tax_id->Visible = false; // Disable update for API request
            } else {
                $this->tax_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->departement__d->CurrentValue = $this->departement__d->FormValue;
        $this->company_id->CurrentValue = $this->company_id->FormValue;
        $this->type->CurrentValue = $this->type->FormValue;
        $this->document_id->CurrentValue = $this->document_id->FormValue;
        $this->document_item_id->CurrentValue = $this->document_item_id->FormValue;
        $this->tax_id->CurrentValue = $this->tax_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // departement__d
        $this->departement__d->RowCssClass = "row";
        ...
        // document_item_id
        $this->document_item_id->RowCssClass = "row";

        // tax_id
        $this->tax_id->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // departement__d
            $this->departement__d->setupEditAttributes();
            $this->departement__d->EditValue = HtmlEncode($this->departement__d->CurrentValue);
            $this->departement__d->PlaceHolder = RemoveHtml($this->departement__d->caption());
            if (strval($this->departement__d->EditValue) != "" && is_numeric($this->departement__d->EditValue)) {
                $this->departement__d->EditValue = FormatNumber($this->departement__d->EditValue, null);
            }
            ...
            // document_item_id
            $this->document_item_id->setupEditAttributes();
            $this->document_item_id->EditValue = HtmlEncode($this->document_item_id->CurrentValue);
            $this->document_item_id->PlaceHolder = RemoveHtml($this->document_item_id->caption());
            if (strval($this->document_item_id->EditValue) != "" && is_numeric($this->document_item_id->EditValue)) {
                $this->document_item_id->EditValue = FormatNumber($this->document_item_id->EditValue, null);
            }

            // tax_id
            $this->tax_id->setupEditAttributes();
            $this->tax_id->EditValue = HtmlEncode($this->tax_id->CurrentValue);
            $this->tax_id->PlaceHolder = RemoveHtml($this->tax_id->caption());
            if (strval($this->tax_id->EditValue) != "" && is_numeric($this->tax_id->EditValue)) {
                $this->tax_id->EditValue = FormatNumber($this->tax_id->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->departement__d->Visible && $this->departement__d->Required) {
            if (!$this->departement__d->IsDetailKey && EmptyValue($this->departement__d->FormValue)) {
                $this->departement__d->addErrorMessage(str_replace("%s", $this->departement__d->caption(), $this->departement__d->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->departement__d->FormValue)) {
            $this->departement__d->addErrorMessage($this->departement__d->getErrorMessage(false));
        }
        ...
        if ($this->document_item_id->Visible && $this->document_item_id->Required) {
            if (!$this->document_item_id->IsDetailKey && EmptyValue($this->document_item_id->FormValue)) {
                $this->document_item_id->addErrorMessage(str_replace("%s", $this->document_item_id->caption(), $this->document_item_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->document_item_id->FormValue)) {
            $this->document_item_id->addErrorMessage($this->document_item_id->getErrorMessage(false));
        }
        if ($this->tax_id->Visible && $this->tax_id->Required) {
            if (!$this->tax_id->IsDetailKey && EmptyValue($this->tax_id->FormValue)) {
                $this->tax_id->addErrorMessage(str_replace("%s", $this->tax_id->caption(), $this->tax_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->tax_id->FormValue)) {
            $this->tax_id->addErrorMessage($this->tax_id->getErrorMessage(false));
        }
    ...
    protected function editRow()
    {
        ...
        // departement__d
        $this->departement__d->setDbValueDef($rsnew, $this->departement__d->CurrentValue, $this->departement__d->ReadOnly);
        ...
        // document_item_id
        $this->document_item_id->setDbValueDef($rsnew, $this->document_item_id->CurrentValue, $this->document_item_id->ReadOnly);

        // tax_id
        $this->tax_id->setDbValueDef($rsnew, $this->tax_id->CurrentValue, $this->tax_id->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("DocumentItemTaxesList"), "", $this->TableVar, true);
        $pageId = "edit";
