 ...
 * Page class
 */
class BeritasEdit extends Beritas
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BeritasEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "BeritasEdit";
    ...
    protected function editRow()
    {
        ...
        // kategori_berita_id
        $this->kategori_berita_id->setDbValueDef($rsnew, $this->kategori_berita_id->CurrentValue, $this->kategori_berita_id->ReadOnly);

        // tanggal_terbit
        $this->tanggal_terbit->setDbValueDef($rsnew, UnFormatDateTime($this->tanggal_terbit->CurrentValue, $this->tanggal_terbit->formatPattern()), $this->tanggal_terbit->ReadOnly);

        // excerpts
        $this->excerpts->setDbValueDef($rsnew, $this->excerpts->CurrentValue, $this->excerpts->ReadOnly);
        ...
        // isi_berita
        $this->isi_berita->setDbValueDef($rsnew, $this->isi_berita->CurrentValue, $this->isi_berita->ReadOnly);

        // gbr_berita
        if ($this->gbr_berita->Visible && !$this->gbr_berita->ReadOnly && !$this->gbr_berita->Upload->KeepFile) {
            $this->gbr_berita->Upload->DbValue = $rsold['gbr_berita']; // Get original value
        ...
        // headline
        $this->headline->setDbValueDef($rsnew, strval($this->headline->CurrentValue) == "1" ? "1" : "0", $this->headline->ReadOnly);
        ...
        if ($this->gbr_berita->Visible && !$this->gbr_berita->Upload->KeepFile) {
            ...
            if (!EmptyValue($this->gbr_berita->Upload->FileName)) {
                ...
                $this->gbr_berita->setDbValueDef($rsnew, $this->gbr_berita->Upload->FileName, $this->gbr_berita->ReadOnly);
            }
        }
