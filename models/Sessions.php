...
class Sessions extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // last_activity
        $this->last_activity = new DbField(
            $this, // Table
            'x_last_activity', // Variable name
            'last_activity', // Name
            '`last_activity`', // Expression
            '`last_activity`', // Basic search expression
            3, // Type
            11, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`last_activity`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->last_activity->InputTextType = "text";
        $this->last_activity->Nullable = false; // NOT NULL field
        $this->last_activity->Required = true; // Required field
        $this->last_activity->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->last_activity->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sessions";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SessionsList");
    }
    ...
    {
        global $Language;
        if ($pageName == "SessionsView") {
            return $Language->phrase("View");
        } elseif ($pageName == "SessionsEdit") {
            return $Language->phrase("Edit");
        } elseif ($pageName == "SessionsAdd") {
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SessionsView", $parm);
        } else {
            $url = $this->keyUrl("SessionsView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SessionsEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SessionsList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SessionsAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SessionsList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl()
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("SessionsDelete");
        }
    }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->last_activity);
                } else {
                    $doc->exportCaption($this->id);
                    $doc->exportCaption($this->user_id);
                    $doc->exportCaption($this->ip_address);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->last_activity);
                    } else {
                        $doc->exportField($this->id);
                        $doc->exportField($this->user_id);
                        $doc->exportField($this->ip_address);
