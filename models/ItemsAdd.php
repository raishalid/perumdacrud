...
class ItemsAdd extends Items
{
    ...
    // CSS class/style
    public $CurrentPageName = "ItemsAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("ItemsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "ItemsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "ItemsView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "ItemsList") {
                            ...
                            $returnUrl = "ItemsList"; // Return list page content
                        }
                    }
    ...
    {
        $this->enabled->DefaultValue = $this->enabled->getDefault(); // PHP
        $this->enabled->OldValue = $this->enabled->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'sale_price' first before field var 'x_sale_price'
        $val = $CurrentForm->hasValue("sale_price") ? $CurrentForm->getValue("sale_price") : $CurrentForm->getValue("x_sale_price");
        if (!$this->sale_price->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->sale_price->Visible = false; // Disable update for API request
            } else {
                $this->sale_price->setFormValue($val, true, $validate);
            }
        }
        ...
        if (!$this->purchase_price->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->purchase_price->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->sale_price->CurrentValue = $this->sale_price->FormValue;
    ...
    public function renderRow()
    {
        ...
        // sale_price
        $this->sale_price->RowCssClass = "row";

        // purchase_price
        $this->purchase_price->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // sale_price
            $this->sale_price->setupEditAttributes();
            $this->sale_price->EditValue = HtmlEncode($this->sale_price->CurrentValue);
            $this->sale_price->PlaceHolder = RemoveHtml($this->sale_price->caption());
            if (strval($this->sale_price->EditValue) != "" && is_numeric($this->sale_price->EditValue)) {
                $this->sale_price->EditValue = FormatNumber($this->sale_price->EditValue, null);
            }

            // purchase_price
            $this->purchase_price->setupEditAttributes();
            ...
            if (strval($this->purchase_price->EditValue) != "" && is_numeric($this->purchase_price->EditValue)) {
                $this->purchase_price->EditValue = FormatNumber($this->purchase_price->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->sale_price->Visible && $this->sale_price->Required) {
            if (!$this->sale_price->IsDetailKey && EmptyValue($this->sale_price->FormValue)) {
                $this->sale_price->addErrorMessage(str_replace("%s", $this->sale_price->caption(), $this->sale_price->RequiredErrorMessage));
            }
        }
        if (!CheckNumber($this->sale_price->FormValue)) {
            $this->sale_price->addErrorMessage($this->sale_price->getErrorMessage(false));
        }
        if ($this->purchase_price->Visible && $this->purchase_price->Required) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        // sale_price
        $this->sale_price->setDbValueDef($rsnew, $this->sale_price->CurrentValue, false);

        // purchase_price
        $this->purchase_price->setDbValueDef($rsnew, $this->purchase_price->CurrentValue, false);
        ...
        $this->enabled->setDbValueDef($rsnew, $tmpBool, strval($this->enabled->CurrentValue) == "");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("ItemsList"), "", $this->TableVar, true);
