 ...
 * Page class
 */
class DocumentHistoriesEdit extends DocumentHistories
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentHistoriesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentHistoriesEdit";
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("DocumentHistoriesList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "DocumentHistoriesList") {
                ...
                if ($this->editRow()) {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "DocumentHistoriesList") {
                            ...
                            $returnUrl = "DocumentHistoriesList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'notify' first before field var 'x_notify'
        $val = $CurrentForm->hasValue("notify") ? $CurrentForm->getValue("notify") : $CurrentForm->getValue("x_notify");
        if (!$this->notify->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->notify->Visible = false; // Disable update for API request
            } else {
                $this->notify->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->notify->CurrentValue = $this->notify->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            // notify
            $this->notify->EditValue = $this->notify->options(false);
            $this->notify->PlaceHolder = RemoveHtml($this->notify->caption());
    ...
    protected function validateForm()
    {
        ...
        if ($this->notify->Visible && $this->notify->Required) {
            if ($this->notify->FormValue == "") {
                $this->notify->addErrorMessage(str_replace("%s", $this->notify->caption(), $this->notify->RequiredErrorMessage));
            }
        }
    ...
    protected function editRow()
    {
        ...
        // departement_id
        $this->departement_id->setDbValueDef($rsnew, $this->departement_id->CurrentValue, $this->departement_id->ReadOnly);
        ...
        // notify
        $tmpBool = $this->notify->CurrentValue;
        if ($tmpBool != "1" && $tmpBool != "0") {
            $tmpBool = !empty($tmpBool) ? "1" : "0";
        }
        $this->notify->setDbValueDef($rsnew, $tmpBool, $this->notify->ReadOnly);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("DocumentHistoriesList"), "", $this->TableVar, true);
        $pageId = "edit";
