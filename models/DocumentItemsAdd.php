 ...
 * Page class
 */
class DocumentItemsAdd extends DocumentItems
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentItemsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentItemsAdd";
    ...
    public function setVisibility()
    {
        ...
        $this->sku->setVisibility();
        $this->quantity->setVisibility();
        $this->price->setVisibility();
        $this->tax->setVisibility();
        $this->discount_type->setVisibility();
        $this->discount_rate->setVisibility();
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'document_items';
        $this->TableName = 'document_items';
        ...
        // Table object (document_items)
        if (!isset($GLOBALS["document_items"]) || get_class($GLOBALS["document_items"]) == PROJECT_NAMESPACE . "document_items") {
            $GLOBALS["document_items"] = &$this;
        }

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'document_items');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "DocumentItemsView"; // If View page, no primary button
                } else { // List page
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("DocumentItemsList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "DocumentItemsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "DocumentItemsView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "DocumentItemsList") {
                            ...
                            $returnUrl = "DocumentItemsList"; // Return list page content
                        }
                    }
    ...
    {
        $this->tax->DefaultValue = $this->tax->getDefault(); // PHP
        $this->tax->OldValue = $this->tax->DefaultValue;
        $this->discount_type->DefaultValue = $this->discount_type->getDefault(); // PHP
        $this->discount_type->OldValue = $this->discount_type->DefaultValue;
        $this->discount_rate->DefaultValue = $this->discount_rate->getDefault(); // PHP
        $this->discount_rate->OldValue = $this->discount_rate->DefaultValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'departemen_id' first before field var 'x_departemen_id'
        $val = $CurrentForm->hasValue("departemen_id") ? $CurrentForm->getValue("departemen_id") : $CurrentForm->getValue("x_departemen_id");
        if (!$this->departemen_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->departemen_id->Visible = false; // Disable update for API request
            } else {
                $this->departemen_id->setFormValue($val, true, $validate);
            }
        }
        ...
        // Check field name 'sku' first before field var 'x_sku'
        $val = $CurrentForm->hasValue("sku") ? $CurrentForm->getValue("sku") : $CurrentForm->getValue("x_sku");
        if (!$this->sku->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->sku->Visible = false; // Disable update for API request
            } else {
                $this->sku->setFormValue($val);
            }
        }
        ...
        if (!$this->tax->IsDetailKey) {
            if (IsApi() && $val === null) {
                ...
                $this->tax->setFormValue($val, true, $validate);
            }
        }

        // Check field name 'discount_type' first before field var 'x_discount_type'
        $val = $CurrentForm->hasValue("discount_type") ? $CurrentForm->getValue("discount_type") : $CurrentForm->getValue("x_discount_type");
        if (!$this->discount_type->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->discount_type->Visible = false; // Disable update for API request
            } else {
                $this->discount_type->setFormValue($val);
            }
        }

        // Check field name 'discount_rate' first before field var 'x_discount_rate'
        $val = $CurrentForm->hasValue("discount_rate") ? $CurrentForm->getValue("discount_rate") : $CurrentForm->getValue("x_discount_rate");
        if (!$this->discount_rate->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->discount_rate->Visible = false; // Disable update for API request
            } else {
                $this->discount_rate->setFormValue($val, true, $validate);
            }
        }
    ...
    public function loadRowValues($rs = null)
    {
        ...
        $this->departemen_id->setDbValue($row['departemen_id']);
        $this->company_id->setDbValue($row['company_id']);
        $this->item_id->setDbValue($row['item_id']);
        $this->name->setDbValue($row['name']);
        $this->description->setDbValue($row['description']);
        $this->sku->setDbValue($row['sku']);
        $this->quantity->setDbValue($row['quantity']);
        $this->price->setDbValue($row['price']);
        $this->tax->setDbValue($row['tax']);
        $this->discount_type->setDbValue($row['discount_type']);
        $this->discount_rate->setDbValue($row['discount_rate']);
    ...
    protected function newRow()
    {
        ...
        $row['departemen_id'] = $this->departemen_id->DefaultValue;
        $row['company_id'] = $this->company_id->DefaultValue;
        $row['item_id'] = $this->item_id->DefaultValue;
        $row['name'] = $this->name->DefaultValue;
        $row['description'] = $this->description->DefaultValue;
        $row['sku'] = $this->sku->DefaultValue;
        $row['quantity'] = $this->quantity->DefaultValue;
        $row['price'] = $this->price->DefaultValue;
        $row['tax'] = $this->tax->DefaultValue;
        $row['discount_type'] = $this->discount_type->DefaultValue;
        $row['discount_rate'] = $this->discount_rate->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        // departemen_id
        $this->departemen_id->RowCssClass = "row";
        ...
        // discount_type
        $this->discount_type->RowCssClass = "row";

        // discount_rate
        $this->discount_rate->RowCssClass = "row";
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->departemen_id->ViewValue = FormatNumber($this->departemen_id->ViewValue, $this->departemen_id->formatPattern());
            ...
            // sku
            $this->sku->ViewValue = $this->sku->CurrentValue;
            ...
            $this->tax->ViewValue = FormatNumber($this->tax->ViewValue, $this->tax->formatPattern());

            // discount_type
            $this->discount_type->ViewValue = $this->discount_type->CurrentValue;

            // discount_rate
            $this->discount_rate->ViewValue = $this->discount_rate->CurrentValue;
            $this->discount_rate->ViewValue = FormatNumber($this->discount_rate->ViewValue, $this->discount_rate->formatPattern());
            ...
            // departemen_id
            $this->departemen_id->setupEditAttributes();
            $this->departemen_id->EditValue = HtmlEncode($this->departemen_id->CurrentValue);
            $this->departemen_id->PlaceHolder = RemoveHtml($this->departemen_id->caption());
            if (strval($this->departemen_id->EditValue) != "" && is_numeric($this->departemen_id->EditValue)) {
                $this->departemen_id->EditValue = FormatNumber($this->departemen_id->EditValue, null);
            }
            ...
            // sku
            $this->sku->setupEditAttributes();
            if (!$this->sku->Raw) {
                $this->sku->CurrentValue = HtmlDecode($this->sku->CurrentValue);
            }
            $this->sku->EditValue = HtmlEncode($this->sku->CurrentValue);
            $this->sku->PlaceHolder = RemoveHtml($this->sku->caption());
            ...
            if (strval($this->tax->EditValue) != "" && is_numeric($this->tax->EditValue)) {
                $this->tax->EditValue = FormatNumber($this->tax->EditValue, null);
            }

            // discount_type
            $this->discount_type->setupEditAttributes();
            if (!$this->discount_type->Raw) {
                $this->discount_type->CurrentValue = HtmlDecode($this->discount_type->CurrentValue);
            }
            $this->discount_type->EditValue = HtmlEncode($this->discount_type->CurrentValue);
            $this->discount_type->PlaceHolder = RemoveHtml($this->discount_type->caption());

            // discount_rate
            $this->discount_rate->setupEditAttributes();
            $this->discount_rate->EditValue = HtmlEncode($this->discount_rate->CurrentValue);
            $this->discount_rate->PlaceHolder = RemoveHtml($this->discount_rate->caption());
            if (strval($this->discount_rate->EditValue) != "" && is_numeric($this->discount_rate->EditValue)) {
                $this->discount_rate->EditValue = FormatNumber($this->discount_rate->EditValue, null);
            }
    ...
    protected function validateForm()
    {
        ...
        if ($this->departemen_id->Visible && $this->departemen_id->Required) {
            if (!$this->departemen_id->IsDetailKey && EmptyValue($this->departemen_id->FormValue)) {
                $this->departemen_id->addErrorMessage(str_replace("%s", $this->departemen_id->caption(), $this->departemen_id->RequiredErrorMessage));
            }
        }
        if (!CheckInteger($this->departemen_id->FormValue)) {
            $this->departemen_id->addErrorMessage($this->departemen_id->getErrorMessage(false));
        }
        ...
        if ($this->sku->Visible && $this->sku->Required) {
            if (!$this->sku->IsDetailKey && EmptyValue($this->sku->FormValue)) {
                $this->sku->addErrorMessage(str_replace("%s", $this->sku->caption(), $this->sku->RequiredErrorMessage));
            }
        }
        ...
        if ($this->tax->Visible && $this->tax->Required) {
        ...
        if ($this->discount_type->Visible && $this->discount_type->Required) {
            if (!$this->discount_type->IsDetailKey && EmptyValue($this->discount_type->FormValue)) {
                $this->discount_type->addErrorMessage(str_replace("%s", $this->discount_type->caption(), $this->discount_type->RequiredErrorMessage));
            }
        }
        if ($this->discount_rate->Visible && $this->discount_rate->Required) {
            if (!$this->discount_rate->IsDetailKey && EmptyValue($this->discount_rate->FormValue)) {
                $this->discount_rate->addErrorMessage(str_replace("%s", $this->discount_rate->caption(), $this->discount_rate->RequiredErrorMessage));
            }
        }
        if (!CheckNumber($this->discount_rate->FormValue)) {
            $this->discount_rate->addErrorMessage($this->discount_rate->getErrorMessage(false));
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // departemen_id
        $this->departemen_id->setDbValueDef($rsnew, $this->departemen_id->CurrentValue, false);
        ...
        // sku
        $this->sku->setDbValueDef($rsnew, $this->sku->CurrentValue, false);
        ...
        // tax
        $this->tax->setDbValueDef($rsnew, $this->tax->CurrentValue, strval($this->tax->CurrentValue) == "");

        // discount_type
        $this->discount_type->setDbValueDef($rsnew, $this->discount_type->CurrentValue, strval($this->discount_type->CurrentValue) == "");

        // discount_rate
        $this->discount_rate->setDbValueDef($rsnew, $this->discount_rate->CurrentValue, strval($this->discount_rate->CurrentValue) == "");
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("DocumentItemsList"), "", $this->TableVar, true);
